...
.flowrow{margin:.9rem 0 1.4rem}
.flowrow .lbl{font-size:.75rem;color:var(--dim);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.45rem}
/* Grilla de columnas fijas: los bloques quedan parejos y la flecha se puede
   apagar en la última columna, que es donde apuntaría al vacío. */
.flow{display:grid;grid-template-columns:repeat(4,1fr);gap:.6rem 1.1rem}
.node{position:relative;background:#12151b;border:1px solid var(--line);border-left:3px solid var(--line);border-radius:9px;padding:.45rem .7rem}
/* La flecha vive en el hueco entre bloques, así el orden se lee sin numerar. */
.node:not(:last-child)::after{content:"→";position:absolute;right:-1.1rem;top:50%;transform:translate(50%,-50%);color:var(--dim);font-size:.95rem;line-height:1}
.node:nth-child(4n)::after{content:none}
.node .kind{display:block;font-size:.68rem;text-transform:uppercase;letter-spacing:.05em;color:var(--dim)}
.node .blk{display:block;font-family:ui-monospace,Menlo,monospace;font-size:.88rem;font-weight:600;color:var(--tx)}
.node .from{display:block;font-size:.76rem;color:var(--dim)}
.node.io{border-left-color:#5b6373}
.node.io .blk{color:var(--dim)}
.node.boost .blk,.node.boost{border-left-color:#c9a0ff}
.node.comp{border-left-color:#8ecbff}
.node.drive{border-left-color:var(--acc)}
.node.amp{border-left-color:#ff5f5f}
.node.cab{border-left-color:#e0a34c}
.node.eq{border-left-color:#a8b3c4}
.node.mod{border-left-color:#7fd6a5}
.node.delay{border-left-color:var(--acc2)}
.node.reverb{border-left-color:#4c8fff}
@media(max-width:820px){
.flow{grid-template-columns:repeat(2,1fr)}
.node:nth-child(4n)::after{content:"→"}
.node:nth-child(2n)::after{content:none}
}
/* En una sola columna la flecha baja al hueco de abajo. Misma especificidad que
   la regla base, si no el top/transform de arriba le gana y se monta al texto. */
@media(max-width:560px){
.flow{grid-template-columns:1fr;gap:1.3rem}
.node:not(:last-child)::after{content:"→";right:auto;left:1.2rem;top:auto;bottom:-1.3rem;transform:translateY(50%) rotate(90deg)}
        ...
        for board in pedalboards["boards"]:
            ...
            if chain.get("rows"):
                ...
                for row in chain["rows"]:
                    p.append('<div class="flowrow">')
                    if row.get("label"):
                        p.append(f'<div class="lbl">{html.escape(row["label"])}</div>')
                    p.append('<div class="flow">')
                    for st in row.get("steps", []):
                        p.append(f'<div class="node {html.escape(st.get("slot","fx"))}">'
                                 f'<span class="kind">{html.escape(st.get("kind",""))}</span>'
                                 f'<span class="blk">{html.escape(st["block"])}</span>'
                                 f'<span class="from">{html.escape(st.get("from",""))}</span></div>')
                    p.append("</div></div>")
